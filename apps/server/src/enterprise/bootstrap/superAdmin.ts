...
const resolveBootstrapPassword = async (
  params: BootstrapSuperAdminParams,
): Promise<{ oneTimePassword?: string; password: string; passwordHash: string }> => {
  ...
  return { oneTimePassword, password, passwordHash };
};

const createBreakGlassSuperAdmin = async (
  db: LobeChatDatabase,
  params: BootstrapSuperAdminParams,
  email: string,
  ...
  const { oneTimePassword, passwordHash } = await resolveBootstrapPassword(params);
...
const promoteExistingSuperAdmin = async (
  ...
  if (needsCredential) {
    ...
    const resolved = await resolveBootstrapPassword(params);
    oneTimePassword = resolved.oneTimePassword;
...
export const bootstrapSuperAdmin = async (
  ...
  if (!userId && params.allowCreate) {
    return createBreakGlassSuperAdmin(db, params, requestedEmail!);
  }
  ...
  return promoteExistingSuperAdmin(db, userId, params);
};
