...
export const MAX_REGEX_PROBES_PER_SAVE = 500;
 ...
 * probe only for patterns that changed vs the persisted config.
 */
export const assertKeywordRegexesSafe = async (params: {
  ...
  const previousPatterns = new Set(
    (params.previous ?? []).filter((rule) => rule.isRegex).map((rule) => rule.pattern),
  );
  ...
  const changed = enabledRegex.filter(({ rule }) => !previousPatterns.has(rule.pattern));
  if (changed.length > MAX_REGEX_PROBES_PER_SAVE) {
    throwEnterpriseError({
      code: PLATFORM_ERROR_CODES.PLATFORM_CONFIG_VALIDATION_FAILED,
  ...
  for (const { index, rule } of changed) {
    const probe = await probeRegexPattern(rule.pattern, { timeoutMs: 200 });
    if (probe.ok) continue;
    rejectKeywordRegex(index, probe.reason === 'invalid' ? 'regex_unsafe' : 'regex_slow');
  }
};
