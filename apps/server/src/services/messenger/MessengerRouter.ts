...
interface MessengerCommand {
  description: string | ((platformName: string) => string);
...
const helpText = (platformName: string) =>
  [
    'Commands:',
    `• /start — bind (or rebind) your ${platformName} account`,
    '• /switch — switch the active scope (personal or a workspace)',
    '• /agents — list your agents and switch the active one',
    '• /new — start a new conversation',
    '• /stop — stop the current execution',
    `• /feedback <message> — send feedback to the ${platformName} team (no AI reply)`,
  ].join('\n');
...
export class MessengerRouter {
  ...
  private buildCommands(): MessengerCommand[] {
    return [
      {
        description: (platformName) => `Bind your account to ${platformName}`,
        handler: async (ctx) => {
          ...
          if (ctx.link) {
            const platformName = await this.resolvePlatformName();
            await ctx.reply(
              `Your account is already linked to ${platformName}. Send /agents to switch the active agent, or /new to start a fresh conversation.`,
            );
            return;
          }
          ...
          if (!ctx.isDM && !canEphemeralInChannel) {
            const platformName = await this.resolvePlatformName();
            await ctx.reply(`Check your DM with ${platformName} for the link button.`);
          }
        },
        name: 'start',
      },
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            ...
            const platformName = await this.resolvePlatformName();
            await ctx.reply(
              `Open your direct message with the ${platformName} bot and send \`/new\` there.`,
            );
            return;
          }
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            const platformName = await this.resolvePlatformName();
            await ctx.reply(
              `Open your direct message with the ${platformName} bot and send \`/stop\` there.`,
            );
            return;
          }
      ...
      },
      {
        description: (platformName) =>
          `Send feedback directly to the ${platformName} team (no AI reply)`,
  ...
  private async runAgentsCommand(ctx: MessengerCommandContext): Promise<void> {
    ...
    if (userAgents.length === 0) {
      const platformName = await this.resolvePlatformName();
      await ctx.reply(
        `You have no agents yet. Create one in ${platformName}, then come back to /agents.`,
      );
      return;
    }
  ...
  private async handleAppHomeOpened(
    ...
    try {
      ...
      const text = activeAgentName
        ? `Welcome to ${platformName}! Your active agent is *${activeAgentName}*. Send a message to chat, or use \`/agents\` to switch.`
        : `Welcome to ${platformName}! Send \`/agents\` to pick an active agent and start chatting.`;
  ...
  private async handleChannelJoin(
    ...
    const platformName = await this.resolvePlatformName();
    const text = [
      ":wave: Hi, I'm your AI agent on Slack.",
      '',
      '• Mention the bot with your question to chat in this channel.',
      `• First time? Send me a *direct message* to link your ${platformName} account.`,
