...
const ExperiencesArea = memo(() => {
  ...
  // A layout effect, and declared above the `useFetchExperiences` call below, so it
  ...
  useLayoutEffect(() => {
    resetExperiencesList(listQuery, epoch);
  }, [epoch, listQuery, resetExperiencesList]);
  ...
  const {
    ...
    isLoading,
    mutate: revalidate,
  } = useFetchExperiences({ ...listQuery, epoch, page: experiencesPage, pageSize: 12 });
  ...
  const handleRetry = useCallback(() => {
    ...
    resetExperiencesList(listQuery, epoch);
    void revalidate();
  }, [epoch, listQuery, resetExperiencesList, revalidate]);
