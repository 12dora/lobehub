...
const assertKeys = (
  ...
  /** When set, zh-CN values must not equal these English placeholders. */
  forbidEnglish?: Record<string, string>,
) => {
  ...
  for (const key of keys) {
    ...
    if (locale === 'zh-CN' && forbidEnglish?.[key] && value === forbidEnglish[key]) {
...
describe('shipped screen locale keys (XC-I18N-003 / CS-02 / XC-I18N-004)', () => {
  ...
  it('setting managed resource names are translated in zh-CN', () => {
    ...
    assertKeys('zh-CN', 'setting', SETTING_MANAGED_RESOURCE_KEYS, {
      'managedResources.resource.agents': 'Agent',
      'managedResources.resource.aiModels': 'Model',
      'managedResources.resource.aiProviders': 'Provider',
      'managedResources.resource.connectors': 'Connector',
      'managedResources.resource.skills': 'Skill',
    });
  });
});
