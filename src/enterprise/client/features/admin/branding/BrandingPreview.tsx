...
export interface BrandingPreviewProps {
  ...
  /** Effective runtime branding used as fallback so an empty draft previews current reality. */
  effective?: RuntimeBranding;
  title: string;
}

export const BrandingPreview = memo<BrandingPreviewProps>(({ copy, draft, effective, title }) => {
  const source = useMemo(() => {
    const name = escapeHtml(draft.name ?? effective?.name ?? copy.defaultName);
    const shortName = escapeHtml(draft.shortName ?? effective?.shortName ?? name);
    const logoUrl = draft.logoUrl ?? effective?.logoUrl;
    const logo = logoUrl
    ...
    const agent = escapeHtml(
      draft.defaultAgentDisplayName ?? effective?.defaultAgentDisplayName ?? `${name} AI`,
    );
    const pageTitle = escapeHtml(
      (draft.pageTitleTemplate ?? effective?.pageTitleTemplate ?? `%s · ${name}`).replace(
        '%s',
        copy.workspace,
      ),
    );
