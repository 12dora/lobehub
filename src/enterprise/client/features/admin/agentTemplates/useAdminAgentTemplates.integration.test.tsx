...
import {
  type PlatformAgentTemplatesState,
  usePlatformAgentTemplates,
} from '@/enterprise/client/hooks/usePlatformAgentTemplates';
 ...
 * only, and the create-agent modal's hook is unmounted the whole time the operator is on an admin
 ...
 * reader is mounted and unmounted, the way opening and closing the modal behaves.
 */
...
let frames: PlatformAgentTemplatesState[] = [];
let cache: Map<unknown, unknown>;
let showReader!: (open: boolean) => void;

const Reader = () => {
  frames.push(usePlatformAgentTemplates());
  return null;
};
...
describe('refreshAdminAgentTemplateLists against a real SWR cache', () => {
  it('serves the operator fresh examples after an admin write, with no stale frame', async () => {
    // 1. The operator opens the create-agent modal once: the served list lands in the cache.
    mountApp();
    await waitFor(() => expect(current().templates).toHaveLength(1));
    expect(current().templates[0]!.title).toBe('Before the edit');

    // 2. They close it and go to the admin console — the reader unmounts, the entry stays.
    ...
    // 4. They reopen the modal. Nothing stale may be rendered on the way to the fresh list: with
    //    a revalidate-only invalidation the first frame here carried "Before the edit".
    frames = [];
    act(() => showReader(true));
    const beforeTheFetchLands = [...frames];

    // The point of the fix: the reopened modal starts from nothing, never from the old rows.
    ...
    // effect); `usePlatformAgentTemplates` must still report "unknown" for that frame so a
    // managed tenant never flashes the built-in locale examples.
  ...
  it('refreshes a still-mounted reader in place instead of stranding it empty', async () => {
    // The console and the modal can be open at once; a mounted reader must land on the new data,
