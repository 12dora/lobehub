 ...
 */
export type AgentTemplateReloadResult =
  | { item: AdminAgentTemplateItem; status: 'found' }
  | { status: 'deleted' }
  | { status: 'unverified' };

/** Rows one recovery read may scan — the admin list contract's own ceiling. */
export const AGENT_TEMPLATE_RECOVERY_SCAN_LIMIT = 100;
 ...
 *
 * There is no `admin.agentTemplates.getById` procedure, so this searches by `identifier`, which is
 ...
 */
export const reloadAgentTemplate = async (
  stale: AdminAgentTemplateItem,
  list: AgentTemplateListReader = (input) => adminAgentTemplatesService.list(input),
): Promise<AgentTemplateReloadResult> => {
  let page: AdminAgentTemplateListOutput;
  try {
    page = await list({
