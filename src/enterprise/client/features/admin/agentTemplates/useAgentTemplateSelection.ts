 ...
 */
export const AGENT_TEMPLATE_SELECTION_COLUMN_WIDTH = 40;

/**
 * Bulk-selection state for the agent-template table.
 *
 ...
 */
export const useAgentTemplateSelection = () => {
  const [selectedMap, setSelectedMap] = useState<Record<string, AdminAgentTemplateItem>>({});

  const selectedRows = useMemo(() => Object.values(selectedMap), [selectedMap]);

  const clearSelection = useCallback(() => setSelectedMap({}), []);

  const rowSelection = useMemo<TableRowSelection<AdminAgentTemplateItem>>(
    () => ({
      columnWidth: AGENT_TEMPLATE_SELECTION_COLUMN_WIDTH,
      preserveSelectedRowKeys: true,
      selectedRowKeys: Object.keys(selectedMap),
      onChange: (keys, rows) => {
        setSelectedMap((prev) => {
          const next: Record<string, AdminAgentTemplateItem> = {};
