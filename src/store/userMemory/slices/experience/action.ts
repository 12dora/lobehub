...
export class ExperienceActionImpl {
   ...
   */
  #adoptableExperiencesEpoch = (
    params: ExperienceQueryParams,
    page: number,
  ): number | undefined => {
    const state = this.#get();
    if (experienceQueryKey(params) !== state.experiencesQueryKey) return undefined;
    if (params.epoch === state.experiencesEpoch) return params.epoch;

    const epoch = params.epoch;
    const isUnadoptedRemount =
      typeof epoch === 'number' && epoch > state.experiencesEpoch && page === 1;
  ...
  #applyExperiencesPage = (
    ...
    const epoch = this.#adoptableExperiencesEpoch(params, page);
