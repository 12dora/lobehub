...
const ensurePendingM09Schema = async () => {
  const statements = [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS "m09_test_binding_owner_unique"
      ON "platform_user_connector_bindings" ("id", "user_id", "connector_id", "published_revision")`,
  ];
...
const ensurePendingM09Constraints = async () => {
  const statements = [
    ...
    `ALTER TABLE "platform_user_connector_bindings" ADD CONSTRAINT "m09_test_binding_revision_check"
      CHECK ("published_revision" > 0 AND "revision" >= 0)`,
    ...
    `ALTER TABLE "platform_connector_oauth_states" ADD CONSTRAINT "m09_test_oauth_owner_fk"
      FOREIGN KEY ("binding_id", "user_id", "connector_id", "published_revision")
      REFERENCES "platform_user_connector_bindings" ("id", "user_id", "connector_id", "published_revision")
      ON DELETE RESTRICT`,
...
describe('PlatformConnectorCatalogRepository', () => {
  ...
  it('atomically consumes a live OAuth state only once under concurrency', async () => {
    ...
    const consumedAt = new Date('2026-07-17T00:05:00Z');
    const attempts = await Promise.all([
      catalog.consumeOAuthState(stateHash, consumedAt),
      catalog.consumeOAuthState(stateHash, consumedAt),
    ]);
    expect(attempts.filter(Boolean)).toHaveLength(1);
    expect(attempts.find(Boolean)).toMatchObject({ consumedAt, id: 'm09-oauth-state' });
    await expect(catalog.consumeOAuthState(stateHash, consumedAt)).resolves.toBeUndefined();
  });
  ...
  it('rejects expired and revoked OAuth states without mutating terminal timestamps', async () => {
    ...
    const consumedAt = new Date('2026-07-17T01:00:00Z');
    await expect(catalog.consumeOAuthState('e'.repeat(64), consumedAt)).resolves.toBeUndefined();
    await expect(catalog.consumeOAuthState('f'.repeat(64), consumedAt)).resolves.toBeUndefined();
  ...
  it('revokes all bindings in bounded idempotent pages and clears token metadata', async () => {
    ...
    for (const [index, userId] of userIds.entries()) {
      await createBinding(userId, connector.id, `m09-binding-${index}`);
    }
    const revokedAt = new Date('2026-07-17T02:00:00Z');
    ...
    await expect(
      catalog.revokeAllBindingsPage({ connectorId: connector.id, limit: 100, revokedAt }),
...
describe('PlatformUserConnectorBindingRepository', () => {
  it('isolates bindings and OAuth state creation by user ownership', async () => {
    ...
    await expect(
      repositoryA.revokeBinding(connector.id, new Date('2026-07-17T00:05:00Z')),
