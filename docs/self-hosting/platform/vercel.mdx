...
This repository deliberately has no GitHub Actions workflow that fetches or merges upstream
changes. To receive future LobeHub updates:
...
5. When you choose to update, review and use GitHub's **Sync fork** / **Update branch** control,
   or merge `upstream/main` from a local clone
