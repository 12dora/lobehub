---
title: LobeHub 手动更新 - Vercel / Zeabur 部署
description: 按照指南重新部署项目，并由维护者按需手动合入上游更新。
tags:
  - Vercel
  - Zeabur
  - 手动更新
...
<Callout type={'warning'}>
  本仓库不包含自动拉取或合并上游改动的 GitHub Actions 工作流。上游更新必须由仓库维护者审核并明确发起。
</Callout>

对于没有冲突改动的 Fork，可以使用 GitHub 的 **Sync fork** 或 **Update branch** 功能。每次更新后请检查引入的提交和部署结果。

对于包含自定义改动的 Fork，请在本地克隆中更新，以便在推送前解决冲突并完成测试：
...
如果已经存在 `upstream` remote，请使用 `git remote set-url upstream
https://github.com/lobehub/lobehub.git` 更新地址，不要重复添加。
