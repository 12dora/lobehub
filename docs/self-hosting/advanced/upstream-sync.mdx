---
title: Manual Upstream Updates and Docker Deployment Guide
description: >-
  Learn how to update a LobeHub fork manually for Vercel deployments and
  upgrade Docker deployment versions.
tags:
  - Manual Updates
  - Vercel Deployment
  - Docker Deployment
...
### Update a Fork Manually

<Callout type={'warning'}>
  This repository does not include a GitHub Actions workflow that fetches or merges upstream
  changes. Upstream updates must be reviewed and started explicitly by a repository maintainer.
</Callout>

For a fork without conflicting changes, use GitHub's **Sync fork** or **Update branch** control.
Review the incoming commits and deployment result after each update.

For forks with custom changes, update from a local clone so conflicts can be resolved and tested
before pushing:
...
If the `upstream` remote already exists, use `git remote set-url upstream
https://github.com/lobehub/lobehub.git` instead of adding it again.
...
<Steps>
  ...
  OPENAI_MODEL_LIST=-gpt-4,-gpt-4-32k,-gpt-3.5-turbo-16k,gpt-3.5-turbo-1106=gpt-3.5-turbo-16k,gpt-4.1=gpt-4-turbo,gpt-4o=gpt-4-vision
  ```
