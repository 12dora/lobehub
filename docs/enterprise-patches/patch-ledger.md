...
| `packages/model-runtime/src/utils/{boundFetch,fetchRequestHandler,azureFetchHttpClient}.ts` + providers bedrock/google/azureai/vertexai + package barrel | SafeOutbound-capable transport for AI connection tests (Bedrock Smithy handler, Google ALS-bound fetch, Azure pipeline httpClient) | M13    | W9 S06 | Server-only at runtime (enterprise connectionTest / custom `fetch`). **W10 fix**: defer `node:stream` and `node:async_hooks` via type-only + dynamic `import()` so SPA production build no longer fails on `Readable` MISSING_EXPORT / top-level Node builtin leakage; Bedrock/Google server behavior unchanged |
