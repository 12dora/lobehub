...
| `CURSOR_AGENT_STATE_DIR`       | `$LOBE_HOME/cursor-agent`，或存在时 `/app/.lobe/cursor-agent`，否则 `<tmpdir>/aihub-cursor-agent` | 可写 0700 目录树，必须是**专用目录**：填 `/`、`/tmp`、`/var`、`/home`、`/root`、`/app` 等根或系统目录会在启动时直接报错（子目录如 `/var/lib/aihub/cursor-agent` 正常）。`home-<实例标识>/`、`cache/` 属于整套 AIHub 安装；`config-seed/account/<sha256 前缀>/` 与 `config-seed/token/<sha256 前缀>/` 按**存储的 Cursor 连接**隔离（见下方）；`turns/<uuid>/` 下的数据、项目、配置与聊天记录只属于当前轮次，结束后删除。 |
...
- 每轮创建 `<stateDir>/turns/<uuid>/config`，启动前只从**该连接自己的**种子目录复制 `cli-config.json` 与 `statsig-cache.json`。种子路径按命名空间拆开：有 `x-aihub-account` 时是 `<stateDir>/config-seed/account/<sha256(accountId) 前 32 位十六进制>/`，没有该头时是 `<stateDir>/config-seed/token/<sha256(bearerToken) 前 32 位十六进制>/`（目录名永远是摘要，两个命名空间在路径上隔离，即使 token 恰好等于某个 accountId 也不会撞车）。`accountId` 与 ChatGPT Web 相同：`platform:<providerId>[:rev:<n>]` / `user:<userId>:<workspace>:<providerId>`，经私有请求头传到传输层后立即剥掉，绝不进入子进程环境变量、argv、历史文件或日志。**不会**退回升级前那个全局 `config-seed/`。轮次结束后只把这两个文件以临时文件加原子重命名的方式写回，并且按**版本比对**（复制时记下种子文件的 sha256，写回前重新计算）：种子在这一轮期间被别的轮次改过就直接放弃写回，避免用旧快照覆盖新结果（原子重命名只能防止读到半个文件，防不了覆盖丢更新）。`chats/**`、`CURSOR_DATA_DIR` 与 `CURSOR_PROJECTS_DIR` 从不进入种子，随轮次根目录一起删除。升级时若全局 `config-seed/cli-config.json`（或 `statsig-cache.json`）仍在、且能确认当前是未钉死 revision 的平台共享连接，会按文件独立、先写 0600 临时文件再用 `link` 无覆盖安装进该连接的 `account/` 目录（目标已存在则跳过，避免覆盖更新的 copy-back；缺的那一个下次再补）；原目录不删。增长按「跑过轮次的连接数」一份摘要目录计，没有旁路索引；退役目录与 `home-*` 一样留给运维手动清理。目录权限 0700（`authInfo` 含 email、userId、teamId、authId 等身份字段，不是 access/refresh token —— token 仍只走进程内 `CURSOR_AUTH_TOKEN` + `AGENT_CLI_CREDENTIAL_STORE=memory`）。
