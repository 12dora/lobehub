...
**Dispose order:** fence (`lifecycle` + `revision++`) and tombstone the jar immediately → synchronous `onBrowserSessionBeforeDispose` (ChatGPT retires the namespaced `ctx:<sha256>` digest) → **defer** every drain into `Promise.resolve().then(() => drain())` and aggregate with `Promise.allSettled` (bound handle, persistent pool, extra CLI drains, `drainAll`, child-close). A synchronous throw becomes a rejected settled entry. Unlink the jar **only if every drain fulfilled**. On any rejection keep the tombstone, do **not** unlink, log, still fire listeners, and rethrow so `awaitPendingCleanup()` rejects with `AggregateError`. Tombstones stay in a **bounded LRU** (cap 4096).

**Reset:** a module-level mutex (promise chain) plus a `resetting` sentinel raised **synchronously before any await**, including the cold `defaultRegistry === undefined` case. `getBrowserSessionRegistry()` throws `BrowserSessionResettingError` (`code: 'BROWSER_SESSION_RESETTING'`, `retryable: true`) while the sentinel is active and does not create a registry. Overlapping resets queue; each installs exactly one fresh registry after its own cleanup. Profile reset (`resetCookieJars`) **refuses to unlink** when cleanup is unproven (keeps tombstones, forgets tracking so a successor's jars cannot be deleted). Call sites map the resetting error as transient: ModelRuntime → `ProviderNetworkError`; staging rethrows (does not collapse to `undefined`); OAuth verify does not emit `access_token_invalid`; session mint becomes a network retryable; connection tests classify `network`, not a permanent provider failure.
