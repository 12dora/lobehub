...
2. **tRPC server → PostgreSQL/Redis/object storage.** The application trusts configured service
   ...
   failure modes. Transaction and compare-and-set boundaries remain security controls. Redis is also
   the shared authority for administrative mutation rate limiting across server processes.
...
| Denial of service      | Repeated tests, syncs, publications, or restart requests consume resources         | Permission gates, bounded schemas, shared multi-instance admin mutation rate limiter (actor+procedure digest, fail closed without Redis), outbound byte/deadline limits                                                     | Rate-limit windows are fixed server-side defaults; ops still need multi-instance chaos evidence                                                    |
