---
title: 新身份验证方式开发指南
description: 学习如何在 LobeHub 中使用 Better Auth 添加新的身份验证提供者。
tags:
  - 身份验证
  - Better Auth
  - OAuth
...
LobeHub 使用 [Better Auth](https://www.better-auth.com) 作为外部身份验证服务。Better Auth 是一个开源的身份验证库，它提供了一种简单的方式来实现身份验证和授权功能。本文档将介绍如何添加新的 SSO 身份验证提供商。

为了在 LobeHub 中添加新的身份验证提供者（例如添加 GitLab)，你需要完成以下步骤：
...
| `generic` | 通过 Generic OIDC/OAuth 插件实现 | Auth0、Keycloak、Okta 等         |

## 添加新的 SSO 提供商

以添加 **GitLab**（代码库中尚未实现）为例，介绍添加 `generic` 类型提供商的完整步骤。

### 步骤 1: 创建提供商定义文件

在 `src/libs/better-auth/sso/providers/` 目录下创建 `gitlab.ts`：
...
在 `packages/env/src/auth.ts` 中添加类型声明：
...
- **Generic OAuth 提供商**：`https://yourdomain.com/api/auth/oauth2/callback/{providerId}`
...
| `packages/env/src/auth.ts`                | 环境变量定义         |
